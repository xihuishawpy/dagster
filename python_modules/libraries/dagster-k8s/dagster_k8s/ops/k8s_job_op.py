...
from ..utils import check_job_status, get_pod_names_in_job, wait_for_job, wait_for_pod
...
@op(
    ...
    config_schema=merge_dicts(
        ...
        {
            "image": Field(
                StringSource, is_required=True, description="The image in which to launch the job."
            ),
            "command": Field([str], is_required=False),
            "args": Field([str], is_required=False),
...
def k8s_job_op(context):
    """
    An op that spins up a Kubernetes job using the k8s API.

    Contrast with the k8s_job_executor, which runs each Dagster op in its own kubernetes job.
